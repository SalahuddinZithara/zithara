    ...
    <section className=" w-full h-auto md:h-auto rounded-md">
      <div className="flex flex-col gap-5 mt-12 w-full">
        {items.map((item, index) => {
          const { id, name, summary, img, path, list } = item;
          return (
            <div
              key={id}
            ...
            >
              ...
              >
                ...
                <div className=" flex flex-col gap-6 w-full h-auto lg:h-auto lg:w-[50%] items-start py-5 pt-10 px-5 lg:pt-10 lg:px-10 max-w-2xl ">
                  <h3 className=" text-2xl md:text-4xl font-bold text-left">
                    {name}
                  </h3>
                  <p className=" font-mediu text-left max-w-md">{summary}</p>
